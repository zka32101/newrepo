...
```mermaid
flowchart TD
    A[ログイン] --> B["デイリーボーナス<br/>コイン +10"]
    B --> C[ホーム画面]
    C --> D1["実験を選ぶ<br/>クイズモード選択（4ステップ×最大4モード）"]
    C --> D2["親子バトル<br/>5ラウンド対戦→親がほめる"]
    C --> D3["ウィークリー<br/>動的ミッション→コイン+50(完全クリア時)"]
    C --> D4["バッジ<br/>17個の進捗表示"]
    C --> D5["キャラ<br/>16体コレクション→コインショップ"]
    C --> D6["成長<br/>月次レポート（棒グラフ）"]
    C --> D7["タイムトラベル<br/>4つのスペシャルミッション"]
    D1 --> E[クイズクリア]
    D2 --> E
    E --> F["親ほめダイアログ<br/>スコア保存→Riverpod更新"]
    F --> G["ほめカード<br/>SNS共有"]
```
